...
char **parse(char *line){
	...
	while (line[next_index_of_line] != '\n'){
		...
		if ((line[next_index_of_line] != ' ') && (line[next_index_of_line] != '\n')){
			rv[curr_arr_size] = (char *)malloc(20 * sizeof(char)); //assuming each word doesnt exceed 20 chars
			while (line[next_index_of_line] != ';' && line[next_index_of_line] != ' ' && line[next_index_of_line] != '\n'){
		...
		if (strcmp(pointer_to_str_arr[i], "exit") == 0){
			...
			if (pid < 0){
				...
				if (execvp(pointer_to_str_arr[i], &(pointer_to_str_arr[i])) == -1)
					printf("Error in child process...check your command\n");
				exit(0);
			}
			else{
				// parent process
